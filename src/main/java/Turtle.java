...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    private class TextToBeShown {
        private String text = "";
        private int x = 0;
        private int y = 0;
        private Font font = null;
        
        TextToBeShown(String text, int xpos, int ypos) {
            this.text = text;
            this.x = xpos;
            this.y = ypos;
        }
        
        TextToBeShown(String text, int xpos, int ypos, Font font) {
            this.text = text;
            this.x = xpos;
            this.y = ypos;
            this.font = font;
    ...
    private static ArrayList<TextToBeShown> textList = new ArrayList<TextToBeShown>();
    ...
    public void showText(String text, int x, int y) {
        TextToBeShown t = new TextToBeShown(text, x, y);
        textList.add(t);
    }

    public void showText(String text, int x, int y, int fontPointSize) {
        Font f = new Font("Serif", Font.PLAIN, fontPointSize);
        TextToBeShown t = new TextToBeShown(text, x, y, f);
        textList.add(t);
    }
 ...
 */
   ...
   /**
    ...
    private static void draw()
    {
        synchronized(turtleLock)
        {
            ...
            for (Map.Entry<Long, ArrayList> entry : turtleStates.tailMap(lastUpdate).headMap(renderTime).entrySet())
            {
                ...
                //t.drawText(offscreen, "Hello World2!", null, 0, 0);
                for(TextToBeShown ts : textList) {
                   t.drawText(offscreen, ts.getText(), ts.getX(), ts.getY(), ts.getFont()) ;
                }
    ...
    private void drawText(Graphics2D g, String text, int x, int y, Font font) {
            if(font==null)  
                font = new Font("Serif", Font.PLAIN, 12);
            g.setFont(font);
            g.drawString(text, (int)((x-centerX)*scale+width/2), (int)((y-centerY)*(-scale)+height/2));           
    }
